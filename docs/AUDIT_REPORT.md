...
- **CI still runs isort/Flake8, not Ruff, for backend linting** (Checkpoint 10 finding) - `.pre-commit-config.yaml` was migrated to Ruff in an earlier pass, but `.github/workflows/testbook-ci.yml`'s `lint-backend` job and `scripts/quality-check.sh` weren't. Both tools are real and currently enforced, just not the same one, which is a real (if narrow) risk of the two disagreeing. Documented in `docs/guides/QUALITY_CHECKS.md` rather than resolved - migrating CI is a workflow change with more consequence than this pass's other documentation fixes, and deserves a maintainer decision (migrate CI to Ruff too, or keep isort/Flake8 in CI deliberately and say why) rather than being picked unilaterally.
