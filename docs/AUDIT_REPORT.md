...
- **The mypy gap noted above is now fixed too** - see §11.
...
## 11. Post-Hardening-Pass Maintenance: 2026-09-09

**Scope:** direct follow-up to §10's two flagged findings (the isort/Flake8-vs-Ruff split, the undeclared mypy dependency), which then expanded at the maintainer's request into a broader sweep for staleness across documentation, config, and pinned tool/dependency versions repo-wide.
**Audience:** the repo maintainer.

This continued straight from §10 rather than starting a new numbered pass in the commit history - 6 commits (`09d6a23` through `afd72fe`), on top of §10's 27.

### What this covered

**The isort/Flake8-vs-Ruff migration** (§10's first finding, now resolved - full detail already recorded in §10's "Decisions" section above, not repeated here): CI's `lint-backend` job, `scripts/quality-check.sh`, and `scripts/verify-release.sh` all run `ruff check .` now. Dead isort/Flake8 config removed, including a root `pyproject.toml` `[tool.flake8]` section that turned out to have never actually been read by real flake8 invocations in this repo at all - flake8 needs a plugin (not installed) to read config from `pyproject.toml`, and every real invocation ran from `backend/` anyway, picking up `backend/.flake8` instead.

**mypy**, §10's second finding: added as a real, declared dependency (`backend/requirements.txt`, `requirements.lock`) rather than left implicit. Checked the real current version via PyPI's API before pinning - 2.3.1 is both what was already installed and verified working in this environment, and the actual latest release, so there was no tension between "use what's proven" and "use what's current."

**The dependency lock file was significantly stale.** Regenerating it (`uv pip compile --upgrade`) to add mypy surfaced that many transitive dependencies unrelated to mypy - cryptography, certifi, pytest, and others - were resolved to old versions, not just the tools this round touched directly. A plain regeneration (no `--upgrade`) even reused a cached resolution pointing at a *yanked* `deprecated` release; `--upgrade` fixed that too. Confirmed nothing in this repo actually installs from `requirements.lock` (no CI job, script, or Dockerfile references it) before doing a full refresh, then verified the refreshed file for real: built a fresh venv, installed directly from it, ran the actual backend suite against it (185 passed, 1 skipped, identical to the primary venv), and confirmed mypy/ruff/black all still pass clean there before deleting it.

**Pre-commit hook versions had zero Dependabot coverage and several were genuinely behind** - `pre-commit-hooks` two major versions (v4.6.0 to v6.0.0), `markdownlint-cli` significantly behind (v0.38.0 to v0.49.1), `ruff-pre-commit` a patch behind (matching the requirements.txt bump, v0.16.4 to v0.16.6). `black`'s hook rev was deliberately *not* bumped to the real current 26.5.1 - it's pinned to match `requirements.txt`'s own `black==25.1.0` exactly, and bumping only the hook side would have recreated the same version-split problem the isort/Flake8-vs-Ruff migration had just fixed, for black instead. Documented that reasoning inline rather than bumping it anyway.

**The markdownlint bump had a real consequence that needed handling, not just accepting**: the old v0.38.0 never enforced MD060 (table column style) at all, so a number of pre-existing table-formatting issues across the repo had only ever been caught by ad-hoc `npx markdownlint-cli2` checks run throughout this repo's audit history, never by the actual pre-commit hook. Upgrading the hook meant `pre-commit run --all-files` would now genuinely fail on those - and since `full-checks.yml` gates the backend/frontend test jobs on pre-commit passing via `pre-commit/action`, this would have broken CI outright, not just added noise. Fixed the real ones: MD060 in 7 files (some with emoji-containing cells, where naive character-count alignment doesn't match markdownlint's own width model - switched those to compact `| --- |` style instead of fighting it), MD051 link-fragment mismatches in two stage READMEs' tables of contents, and two MD032 false positives that were really markdown's own list-marker ambiguity (a wrapped sentence starting a line with "- ", meant as a dash, parsed as an unintended list item). `docs/AUDIT_REPORT.md`'s original historical sections were not touched for this - excluded the file from the markdownlint hook instead, writing down explicitly what had been this repo's informal, repeatedly-stated policy across every prior pass (an audit trail's dated sections stay an honest record of what was true when written, not silently reformatted later).

**Docker base images had no staleness tracking at all** - `dependabot.yml` covered github-actions, npm ×2, and pip ×3, but no `docker` ecosystem entry existed for `Dockerfile` (`python:3.13-slim-bookworm`, `node:24-alpine`) or `.devcontainer/Dockerfile.dev` (`python:3.13-slim-bookworm`). Checked whether the current pins were actually stale before assuming so: Debian 12 "Bookworm" isn't the newest Debian release anymore (13 "Trixie" is), but it's fully supported until 2028 per Debian's real release schedule - pinning to a specific codename for reproducibility is a legitimate deliberate choice, and jumping to Trixie unprompted would have been a judgment call outside this round's scope, not a staleness fix. Added ongoing Dependabot tracking instead, matching how every other dependency type in this repo is already handled.

**Version claims and GitHub Actions pins repo-wide were checked, not assumed current.** Grepped every doc for Python/Node version claims (`Python 3\.[0-9]+`, `Node\.?js\? 2[0-9]`) and confirmed all matched the real pinned `.python-version`/`.nvmrc` values, with the WINDOWS_SETUP.md "Python 3.14"/"3.11" mentions confirmed as deliberate compatibility warnings, not stale claims. Checked every pinned GitHub Action major version (`checkout@v7`, `setup-python@v7`, `setup-node@v7`, `upload-artifact@v7`, `cache@v6`, `github-script@v9`) against GitHub's real releases API - all genuinely current, and internally consistent across every workflow file with no exceptions found. (One open Dependabot PR references `actions/checkout` 4 to 6, which doesn't match what's actually pinned in the repo - almost certainly a PR opened before an earlier pass manually bumped to v7 by other means, now stale/superseded; not something fixable from this environment, since it lives on GitHub, not in the working tree.)

### Real final numbers (this round)

No application code changed, so these match §10's numbers exactly - re-run for confirmation after every change in this round, not assumed to still hold:

| Suite | Result |
| --- | --- |
| Backend (`pytest`) | 185 passed, 1 skipped |
| Frontend (`npm test`) | 144 passed, 2 skipped |
| Markdown links (repo-wide) | 701 links checked, 0 broken |
| `pre-commit run --all-files` | All 14 hooks passed |

### Decisions the maintainer should be aware of

- **The stale-lock-file-revealed-deeper-staleness pattern is worth noticing**: `requirements.lock` isn't consumed by anything today, but if that ever changes (someone starts installing from it, or a CI job starts referencing it for reproducibility), it would have silently pinned old, and in one case yanked, versions. Regenerating it periodically (or wiring a check that flags when it drifts too far from what `requirements.txt` alone would resolve to) would catch this earlier next time.
- **The stale Dependabot PR for `actions/checkout`** (4 to 6, when the repo is actually on v7) is real GitHub state this environment can't act on - worth closing manually, or letting Dependabot's own rebase-on-conflict behavior resolve it once it notices the target has moved past what the PR proposes.
- **Debian Bookworm vs Trixie** is now tracked by Dependabot (this round's fix) but not acted on - deliberately, since it's a real upgrade decision (new Debian major, potential compatibility surface) rather than a mechanical version bump, and belongs to whoever reviews the resulting Dependabot PR, not this maintenance sweep.
